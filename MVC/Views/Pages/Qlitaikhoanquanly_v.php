<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Quản lí tài khoản quản lý</title>
    <link rel="stylesheet" href="http://localhost/rapphim/Public/CSS/qlitaikhoan.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.2/css/all.min.css">
</head>
<body>

    <div class="header">
        <h2>Tài khoản quản lý</h2>
    </div>

    <!-- Nút quay lại trang chủ -->
    <div class="nav-buttons-left">
        <a href="http://localhost/rapphim/HomeQuanly">
            <button><i class="fa-solid fa-house"></i> Quay lại trang chủ</button>
        </a>
    </div>

    <div class="search-add-container">
        <a href="http://localhost/rapphim/Qlitaikhoannhanvienxem/them">
            <button><i class="fa-solid fa-plus"></i> Thêm tài khoản nhân viên</button>
        </a>
        <a href="http://localhost/rapphim/Qlitaikhoannhanvienxem">
            <button><i class="fa-solid fa-users"></i> Xem tài khoản nhân viên</button>
        </a>
        <a href="http://localhost/rapphim/Qlitaikhoankhachhangxem">
            <button><i class="fa-solid fa-users"></i> Xem tài khoản khách hàng</button>
        </a>
    </div>
    <div class="content">
        <table>
            <thead>
                <tr>
                    <th>STT</th>
                    <th>Mã quản lý</th>
                    <th>Tên quản lý</th>
                    <th>Số điện thoại</th>
                    <th>Địa chỉ</th>
                    <th>Email</th>
                    <th>Mật khẩu</th>
                    <th>Thao tác</th>
                </tr>
            </thead>
            <tbody>
                <?php if (!empty($data['dulieu']) && mysqli_num_rows($data['dulieu']) > 0): ?>
                    <?php $i = 0; ?>
                    <?php while ($row = mysqli_fetch_assoc($data['dulieu'])): ?>
                        <tr>
                            <td><?php echo ++$i; ?></td>
                            <td><?php echo htmlspecialchars($row['maQuanly']); ?></td>
                            <td><?php echo htmlspecialchars($row['tenQuanly']); ?></td>
                            <td><?php echo htmlspecialchars($row['soDienthoai']); ?></td>
                            <td><?php echo htmlspecialchars($row['diaChi']); ?></td>
                            <td><?php echo htmlspecialchars($row['Email']); ?></td>
                            <td><?php echo htmlspecialchars($row['matKhau']); ?></td>
                            <td>
                                <a href="http://localhost/rapphim/QlitaikhoanQuanly/sua/<?php echo urlencode($row['maQuanly']); ?>" class="btnsua">
                                    <i class="fas fa-edit"></i> Sửa
                                </a>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="8">Không có dữ liệu quản lý.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>

</body>
</html>
